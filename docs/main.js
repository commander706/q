...
const fsSource = `
  ...
  void main() {
    ...
    // Grid
    vec2 gv = uv;
    gv.x *= 1.0 + (0.5 - gv.y) * 1.5; // perspective
    float gridVal = 0.0;
    if(gv.y < -0.1) {
    ...
    col += stars * (1.0 - step(0.0, uv.y + 0.1)); // Sky only
...
function connectToHost(room) {
  ...
  hostConn.on("open", () => {
    ...
    // Request lobby update explicitly
...
function handleData(data, conn) {
  switch(data.t) {
    ...
    case "updateLobby":
      ...
      renderLobby();
      if($("lobbyArea").classList.contains("hidden") && gameState === "lobby") enterLobby();
      break;
    ...
    case "prepareMatch":
      // Spectators and players both run this
...
function runNextMatch(idx) {
  ...
  if(idx >= tournamentQueue.length) {
    ...
    if(winners.length===1) {
      setTimeout(()=> { alert(`CHAMPION: ${winners[0].name}`); location.reload(); }, 3000);
      return;
    }
    // Next Round
    const nextQ = [];
...
function checkHit(pid, clientTime) {
  ...
  // In a real game, compensate latency. Here, first arrival wins.
  matchActive = false;
  const isP1 = (pid === hostP1);
  const loser = isP1 ? hostP2 : hostP1;
  const pData = players.get(pid);
  
  // Reaction Calculation (approx)
  // We sent signal at `signalTime`. Client replied at `clientTime`.
  // Note: timestamps aren't synced across machines perfectly without NTP.
  // But we can approximate using the host's "now()" as the reference when receiving the packet.
  // Better: Use Host time.
  const hostNow = now();
...
async function runMatchSequence(p1, p2) {
  ...
  // 2. Waiting Phase (Lightning)
  lightning.classList.remove("hidden"); // Quick flash
  setTimeout(()=>lightning.classList.add("hidden"), 200);
}

function triggerSignal(svTime) {
  signalTime = now(); // Host time approx for self, or visuals
...
function showResult(data) {
  ...
  // --- SCREEN SLICE EFFECT ---
  // Clone the view to slice it
  // Trick: We just create two divs covering the screen with the current background/canvas?
  // Easier: Apply clip-path to mainContainer copy.
  // Since mainContainer has the game DOM, let's clone it.
  ...
  if(data.reason === "FALSE START") {
    ...
    if(data.time < 0.15) rank = "GODLIKE (神)";
    else if(data.time < 0.25) rank = "SAMURAI (侍)";
    else if(data.time < 0.35) rank = "SOLDIER (兵)";
    else rank = "TURTLE (亀)";
  }
